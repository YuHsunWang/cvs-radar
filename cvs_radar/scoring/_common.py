...
_OPTIONAL_RE = re.compile(
    ...
    r"(特大杯|大杯|中杯|小杯|口味|數量|容量|規格|加量|限定|新品|新上市))"
)
...
# 品名清理時要剝掉的標點。數字裡的符號是例外，因為它們是品名的一部分：
#   小數點  夾在數字之間時保留（牧場直送4.0、可樂2.5公升），整組剝掉會讓 4.0 變成
#           40，看起來像容量或價格。
#   百分比  接在數字後、後面還有品名時保留（GODIVA 54%黑巧克力）。位在字尾的則是
#           折扣（折5%、優惠10%、50x65%），照樣剝掉。
# 過去是靠 product_overrides 逐項把符號補回去，改成在來源保留。
...
# 有些貼文的商品名稱欄填的是「怎麼拿到」而不是「這是什麼」——如「買六件20元以上冰品
# 飲料免費送」，真正的商品（超級瑪利歐矽膠杯墊）只寫在標題。這種欄位同時具備購買條件
# 與贈送語意，兩者都命中才算取得條件句，單獨出現的「買一送一」「加價購」只是接在真實
# 品名後面的促銷尾巴，那由 _PROMO_* 負責剝除，不該讓整個品名作廢。
_PURCHASE_CONDITION_RE = re.compile(
    r"[買滿][一二三四五六七八九十兩\d]+(?:件|個|元|支|包|杯|入|盒|瓶|份)|"
    r"第[二三四五六七八九十\d]+件|滿額|加價購"
)
_GIVEAWAY_RE = re.compile(r"免費|贈送|贈品|兌換|換購|送$")

# 全家「友善時光」是即期品折扣活動，不是商品名。貼文者常把它連同折扣價填進商品名稱欄。
# 「友善時光」四字不會與真實品名衝突（如「光泉午后時光」是不同字），可整組剝除；至於裸
# 「時光」則只在「價格+時光」的折扣尾巴（如「品名/55 /38時光」）才剝，避免誤傷以「時光」
# 結尾的品名（午后時光…）。
_FRIENDLY_TIME_MARK_RE = re.compile(r"友善時光")
_FRIENDLY_TIME_TAIL_RE = re.compile(r"[/／]?\s*\d{1,3}\s*時光\s*$")
...
# 商品名稱欄的通用形狀是「品名 + 分隔符 + 價格修飾詞 + 價格」。價格被抽走後，修飾詞
# 會黏在品名尾巴（鹽花生焦糖捲蛋糕「友善」、楊枝甘露果C果昔「嚐鮮價」）。這裡只在
# **字串尾端**反覆剝除這組零售促銷詞彙，不做全域刪字，因此品名中段的同形字不受影響
# （牧場直送、開運黑糖發糕、光泉午后時光都安全）。DEV-110。
_TRAILING_QUALIFIER_RE = re.compile(
    # a price may sit between the name and the qualifier (法朋蛋黃酥霜淇淋「49」第二隻);
    # it is only eaten when a qualifier actually follows, so names that genuinely end
    # in digits keep them.
    r"(?P<price>\d{1,3})?\s*元?\s*"
    r"(?:"
    r"友善|嚐鮮價|嘗鮮價|嚐鮮|嘗鮮|促銷價|加購價|優惠價|特惠價|活動價|會員價|"
    r"原價|售價|特價|優惠|促銷|單價|總價|價格|價|"
    r"折價後|打折後|折後|折價|打折|領券|自助結帳|結帳|"
    r"i珍食|惜福|限店|"
    r"第[一兩二三四五六七八九十\d]+[件瓶隻支個杯份]|"
    r"任[一兩二三四五六七八九十\d]*[件瓶隻支個杯份]?折?|任|"
    r"[一兩二三四五六七八九十\d]+折|折|"
    r"商品"
    r")+$"
)

# 斜線後若是「沒有數字的價格敘述」（一起結帳不確定價格 / 價格忘記），那是發文者的
# 附註而非品名。要求整段不含數字，才不會誤砍真正的價格段或第二個商品。
_PRICE_NOTE_ASIDE_RE = re.compile(
    r"[/／｜|，,]\s*[^/／｜|\d]*"
    r"(?:一起結帳|結帳|忘記|不確定|不知道|沒注意|沒記|可能是|大概是|忘了)"
    r"[^/／｜|\d]*$"
)

# 贈品／門檻尾巴：買N送、消費滿N、任N件…免費送贈品。只在價格或分隔符之後成立。
_GIFT_TAIL_RE = re.compile(
    # 「送」必須有前導的分隔符、價格或門檻語才算贈品條款；否則品名裡的「送」會被誤咬
    # （牧場直送生食鮮蛋 → 牧場直）。DEV-110。
    r"(?:"
    # 門檻語（消費滿200／任四件／買一）後接「送…」。刻意不吃掉門檻語前面的數字，
    # 那通常是商品本身的價格（五窨茉莉茶后「40」買一送一）。
    r"[/／、,，+＋]?\s*"
    r"(?:消費滿\d*|任[一兩二三四五六七八九十\d]+[件瓶個支杯份]?|"
    r"買[一兩二三四五六七八九十\d]+[件瓶個支杯份]?|[一兩二三四五六七八九十\d]+件)"
    r"\s*[一兩二三四五六七八九十\d]*折?\s*(?:免費)?送.*"
    # 無門檻語時，「送」前面一定要有分隔符或價格
    r"|[/／、,，+＋]\s*\d{0,3}\s*元?\s*(?:免費)?送.*"
    r"|\d{1,3}\s*元\s*(?:免費)?送.*"
    r")$"
)

# 小數價格（24.5元）只取整數位；品名裡的小數（牧場直送4.0）不接「元」，不受影響。
_DECIMAL_PRICE_RE = re.compile(r"(\d+)\.\d+(?=\s*元)")

# 折扣乘數寫法（79X0.7 / 59*0.75）：價格後面的乘數不是品名的一部分。
_DISCOUNT_MULTIPLIER_RE = re.compile(r"\s*[xX*×]\s*0?\.\d+\s*")
...
_EXCERPT_INTRO_RE = re.compile(r"(今天|昨天|之前|記得|原本|剛剛|下班|路過|逛到|看到|買來|入手|開箱|分享|先上圖|網路說|大家都說|聽說)")

_EXCERPT_REPORTED_OPINION_RE = re.compile(r"(網路說|大家都說|聽說)")
...
_EXCERPT_CONTINUATION_RE = re.compile(r"^(?:帶|並|且|以及|而且)")

_EXCERPT_PRODUCT_FORM_TERMS = (
    "霜淇淋", "冰淇淋", "雪糕", "冰棒", "冰沙", "甜筒", "涼麵",
    "麵包", "餅乾", "脆條", "飯糰", "便當", "飲料",
)
...
_EXCERPT_ASPECT_TERMS: dict[str, tuple[str, ...]] = {
    ...
    # 用料是選購資訊的一種（「果乾的確也是滿滿」）。收一組常見的內容物詞，不要只為
    # 單一商品加詞，否則等於把特例寫死在通用規則裡。
    "ingredient": (
        "果乾", "內餡", "外皮", "餡料", "夾心", "配料", "醬汁", "湯頭", "麵體",
        "肉片", "肉塊", "果肉", "顆粒", "碎粒", "堅果", "起司", "珍珠",
    ),
    # 這兩類原本只收精確詞，漏掉口語寫法（好大一坨／單價90塊／真的好貴），導致有實質
    # 資訊的句子拿不到 aspect 分數，輸給「超級好吃」這種空泛稱讚。DEV-110。
    "portion": (
        "份量", "分量", "內容量", "大小", "飽足", "吃不飽", "給得多", "給的多",
        "太少", "好大", "很大", "超大", "好小", "很小", "一大", "料多", "料少",
    ),
    "value": (
        "價格", "價位", "單價", "划算", "便宜", "偏貴", "太貴", "好貴", "很貴", "不貴",
        "CP值", "cp值", "值得", "塊", "元",
    ),
