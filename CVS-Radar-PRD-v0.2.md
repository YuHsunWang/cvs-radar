...
|---|---|
| 文件版本 | **v0.2**(取代 v0.1) |
| 適用範圍 | v0:PTT `CVS` 板;v1 規劃接入 Dcard |
| 狀態 | 待需求方確認(見 §16 開放問題) |
| 最後更新 | 2026-06-15 |
...
在超商挑食物時,消費者很難快速判斷一個商品好不好吃:資訊散落在 PTT、Dcard;同一商品多篇心得評價可能截然不同,逐篇翻很慢;推文良莠不齊,部分疑似廠商自家帳號互推刷評,降低參考價值;缺乏彙整後的「公正分數」可一眼判斷該不該踩雷。
...
### 2.2 非目標(v0 不做)
完整前端網站/App、即時爬取、公開指控真實帳號、接 Dcard、跨平台帳號串接。
...
- F1.6(P2)抽象 `Source` 介面,供 Dcard 實作接入。
...
- v1 接 Dcard 時 `Post`/`Comment` 結構共用,差異收斂於 `Source` 與各自 crawler/parser。
...
| **v1** | 接 Dcard(F1.6);跨來源彙整;Web dashboard(F7.4)。 |
...
- 各來源分數尺度不同 → 先**各自標準化**再加權合併;來源權重可設定。
- 帳號跨平台**不串接**(非目標);偏好與可疑分於各來源獨立計算。
- 商品跨來源識別:品牌 + 正規化名 + 模糊比對。
- 呈現時可標示各來源分歧(例:PTT 好評但 Dcard 兩極)。
