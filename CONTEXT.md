...
> 本檔只是查找表;規則與流程見 AGENTS.md,此處不重述。
...
- **意圖分類**:兩層,別搞混。
  - **標記層 11 類**(`product_category_labels.csv` 存的值):便當/飲料/甜點/
    麵包/冰品/鹹食/零食/周邊/泡麵/乳品/其他。這些名稱是 load-bearing 的,
    `cvs_radar/scoring/identity.py` 用它們辨認「商品名稱欄位其實只是個類別詞」。
  - **UI 篩選 7 組**(`web/lib/data.ts` 的 `categoryGroups`):正餐/甜點/冰品/
    飲料/麵包/零食/其他。正餐=便當+鹹食+泡麵,飲料=飲料+乳品,
    其他=其他+周邊。**訪客只看得到這 7 個。**
- **品牌 chips**:UI 上可交叉篩選的品牌標籤。
- **通路**:7-11、全家、萊爾富、OK、美廉社及其他。
