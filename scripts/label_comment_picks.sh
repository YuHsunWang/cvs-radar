#!/usr/bin/env bash
# label_comment_picks.sh — label the representative-comment delta with a local Codex CLI.
#
# Mirrors label_product_names.sh: export the products whose candidate pool has no
# pick yet, chunk them, let an LLM choose which comments (and, as a backstop, which
# author sentences) a shopper should see, verify the answers mechanically, then
# import into data/labels/comment_picks.csv.
#
...
log(){ echo "[label-comment-picks] $*"; }
die(){ echo "[label-comment-picks] FAILED: $*" >&2; exit 1; }
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/label-comment-picks.XXXXXX")" || die "mktemp"
...
mkdir -p comment_pick_work/chunks "$WORK/prompts"
rm -f comment_pick_work/chunks/*.csv
python3 - "$DELTA" comment_pick_work/chunks "$CHUNK" <<'PY' || die "chunk"
import csv, sys
...
for f in comment_pick_work/chunks/chunk_*.csv; do
...
# --- 4. label with Codex ---
log "labeling $(ls comment_pick_work/chunks/chunk_*.csv | wc -l) chunk(s), concurrency $CONC"
: > "$WORK/manifest.tsv"
run_one(){
  local nn="$1"
...
# Only structural checks here; scripts/import_comment_picks.py re-validates every
# index against its candidate cell and refuses the file rather than repairing it.
python3 - comment_pick_work/chunks "$WORK/all_labeled.csv" <<'PY' || die "verification failed"
...
for src in sorted(glob.glob(f"{source_dir}/chunk_*.csv")):
    ...
    # The candidate cells are the ground truth the indices point at, so a chunk that
    # rewrote them would silently re-aim every pick.
...
python3 scripts/import_comment_picks.py "$WORK/all_labeled.csv" 2>&1 | tail -1 || die "import"
