...
**11. The sentiment prompt is not in `scripts/prompts/`.** Only the three Codex
layers have prompt files there (category and grounding are Jev questions in
`scripts/jev_labelers.py`); sentiment's canonical rubric is the Jev questions in
